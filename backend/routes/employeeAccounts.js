...
const { handleError, createSuccessResponse } = require('../utils/employeeAccountValidation');
const {
 ...
 */
router.post('/', [auth, adminAuth, createAccountValidation], async (req, res) => {
  try {
 ...
 * @access  Private/Admin
 */
router.put('/:employeeId', [auth, adminAuth, updateAccountValidation], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
