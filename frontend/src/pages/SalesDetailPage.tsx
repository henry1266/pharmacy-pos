  ...
  return (
    <TableRow hover>
      ...
      {!fifoLoading && fifoData?.items && showSalesProfitColumns && (
        <>
          <TableCell align="right">{(fifoItem && typeof fifoItem === 'object') ? fifoItem.fifoProfit?.totalCost?.toFixed(2) ?? 'N/A' : 'N/A'}</TableCell>
          {/* 型別斷言在此是必要的，因為可能存在兩個不同的 FifoProfit 介面定義 */}
          <GrossProfitCell fifoProfit={(fifoItem && typeof fifoItem === 'object') ? fifoItem.fifoProfit as any : undefined} />
          <TableCell align="right" sx={{ color: (fifoItem && typeof fifoItem === 'object') && fifoItem.fifoProfit && parseFloat(fifoItem.fifoProfit.profitMargin || '0') >= 0 ? 'success.main' : 'error.main' }}>
            {(fifoItem && typeof fifoItem === 'object') ? fifoItem.fifoProfit?.profitMargin ?? 'N/A' : 'N/A'}
          </TableCell>
        </>
      )}
    </TableRow>
