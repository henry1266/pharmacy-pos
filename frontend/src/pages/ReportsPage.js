...
const ReportsPage = () => {
  ...
  // Colors and options (remain the same)
  ...
  // Apply Filters (only relevant for manual trigger, now using useEffect)
  // const applyFilters = () => {
  //   if (reportType === 'sales') {
  //     fetchSalesData({ startDate: dateRange.startDate, endDate: dateRange.endDate, groupBy });
  //   }
  ...
  const renderSalesReport = () => {
    ...
    return (
      <>
        ...
        {/* Sales Trend Chart (remain the same structure, uses loading/data from hook) */}
        <Card sx={{ borderRadius: 'var(--border-radius)', boxShadow: 'var(--card-shadow)', mb: 4 }}>
          <CardContent>
            ...
            {loading ? (
              ...
              <Box sx={{ p: 3, textAlign: 'center' }}><Typography color="error">{error}</Typography></Box>
            ) : data.length === 0 ? (
              <Box sx={{ p: 3, textAlign: 'center' }}><Typography color="var(--text-secondary)">暫無數據</Typography></Box>
            ) : (
        ...
        {/* Sales Detail Table (remain the same structure, uses loading/data from hook) */}
        <Card sx={{ borderRadius: 'var(--border-radius)', boxShadow: 'var(--card-shadow)' }}>
          <CardContent>
            ...
            {loading ? (
              ...
              <Box sx={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                  ...
                  <tbody>
                    {data.map((item, index) => (
                      <tr key={index} style={{ backgroundColor: index % 2 === 0 ? 'transparent' : 'rgba(0, 0, 0, 0.02)' }}>
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Container maxWidth="xl" sx={{ py: 4 }}> {/* Use Container for better layout */} 
              ...
              <Grid container spacing={2} alignItems="center">
                  ...
                  <FormControl fullWidth size="small">
                    ...
                    >
                      {dateRangeOptions.map(option => (
                       ...
                       <MenuItem value="custom">自定義</MenuItem> {/* Ensure custom option exists */}
                    </Select>
                  </FormControl>
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <DatePicker
                    ...
                    renderInput={(params) => <TextField {...params} fullWidth size="small" />}
                    inputFormat="yyyy/MM/dd"
                    minDate={dateRange.startDate} // Prevent end date being before start date
                    disabled={selectedDateRangeOption !== 'custom'}
                  />
                </Grid>
                ...
                <Grid item xs={12} md={1} sx={{ display: 'flex', justifyContent: { xs: 'flex-start', md: 'flex-end' } }}>
                 ...
                 {/* Removed Apply Filter button as useEffect handles it */}
                 {/* <Grid item xs={12} md={1} sx={{ display: 'flex', justifyContent: { xs: 'flex-start', md: 'flex-end' } }}>
                   <Button 
                     variant="contained" 
        ...
        {/* Add rendering for other report types here */}
        {/* {reportType === 'accounting' && <AccountingReportComponent />} */}
