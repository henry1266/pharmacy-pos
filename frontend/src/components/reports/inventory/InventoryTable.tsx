...
// 展開行組件
const ExpandableRow: FC<ExpandableRowProps> = ({ item, formatCurrency }) => {
  ...
  // 計算累積庫存和損益總和
  const calculateCumulativeValues = (): Transaction[] => {
                  ...
                  {displayTransactions.map((transaction) => (
                    <TableRow key={`${transaction.type}-${getOrderNumber(transaction)}-${transaction.price}`}>
                      ...
                      <TableCell align="right" sx={{ 
                        ...
                        {formatCurrency(transaction.cumulativeProfitLoss || 0)}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
