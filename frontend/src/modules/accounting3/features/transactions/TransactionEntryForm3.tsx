...
export const TransactionEntryForm3: React.FC<TransactionEntryForm3Props> = ({
  ...
  useEffect(() => {
    if (isCopyMode && entries.length > 0) {
      console.log('🔄 TransactionEntryForm3 - 複製模式，清空分錄描述:', { isCopyMode, entriesCount: entries.length });
      ...
      if (needsClear) {
        ...
        console.log('✅ TransactionEntryForm3 - 清空分錄描述完成');
  ...
  const validationResult = useMemo(() => {
    const baseValidation = embeddedEntriesHelpers.validateEntries(entries);
    // 補充 warnings 屬性以符合 BalanceValidator3 的介面要求
    return {
      ...baseValidation,
      warnings: [] as string[]
    };
...
export default TransactionEntryForm3;