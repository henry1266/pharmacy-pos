  ...
  <Card variant="outlined">
    <CardContent>
      <Typography variant="h6" gutterBottom>
        <InfoIcon sx={{ verticalAlign: 'middle', mr: 1 }}/>基本信息
      </Typography>
      <Divider sx={{ mb: 2 }} />
      <Stack spacing={1.5}>
        ...
        <Stack direction="row" spacing={1} alignItems="center">
          {getIconByType(getPaymentStatusInfo(sale.paymentStatus).iconType)}
          <Typography variant="body2">
            付款狀態:
            <Chip
              label={getPaymentStatusInfo(sale.paymentStatus).text}
              color={getPaymentStatusInfo(sale.paymentStatus).color || 'default'}
              size="small"
              sx={{ ml: 1 }}
            />
          </Typography>
