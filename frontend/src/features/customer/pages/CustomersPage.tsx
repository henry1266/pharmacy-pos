...
const CustomerDetailPanel: FC<CustomerDetailPanelProps> = ({ selectedCustomer, handleEdit, handleDelete }) => {
  ...
  return (
    <Card elevation={2} sx={{ borderRadius: '0.5rem', height: '100%' }}>
      ...
      <Divider />
      <CardContent sx={{ py: 1 }}>
        <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 1 }}>會員資訊</Typography>
        <List dense sx={{ py: 0 }}>
          <ListItem sx={{ py: 0.5, display: 'flex', justifyContent: 'space-between' }}><Typography variant="body2" sx={{ width: '25%', color: 'text.secondary' }}>電話:</Typography><Typography variant="body2" sx={{ width: '80%', fontWeight: 500 }}>{selectedCustomer.phone}</Typography></ListItem>
          <ListItem sx={{ py: 0.5, display: 'flex', justifyContent: 'space-between' }}><Typography variant="body2" sx={{ width: '25%', color: 'text.secondary' }}>身分證:</Typography><Typography variant="body2" sx={{ width: '80%', fontWeight: 500 }}>{selectedCustomer.idCardNumber ?? '無'}</Typography></ListItem>
          <ListItem sx={{ py: 0.5, display: 'flex', justifyContent: 'space-between' }}><Typography variant="body2" sx={{ width: '25%', color: 'text.secondary' }}>生日:</Typography><Typography variant="body2" sx={{ width: '80%', fontWeight: 500 }}>{formatDateToYYYYMMDD(selectedCustomer.birthdate) ?? '無'}</Typography></ListItem>
          <ListItem sx={{ py: 0.5, display: 'flex', justifyContent: 'space-between' }}><Typography variant="body2" sx={{ width: '25%', color: 'text.secondary' }}>Email:</Typography><Typography variant="body2" sx={{ width: '80%', fontWeight: 500 }}>{selectedCustomer.email ?? '無'}</Typography></ListItem>
          <ListItem sx={{ py: 0.5, display: 'flex', justifyContent: 'space-between' }}><Typography variant="body2" sx={{ width: '25%', color: 'text.secondary' }}>等級:</Typography><Typography variant="body2" sx={{ width: '80%', fontWeight: 500 }}>{selectedCustomer.level}</Typography></ListItem>
          <ListItem sx={{ py: 0.5, display: 'flex', justifyContent: 'space-between' }}><Typography variant="body2" sx={{ width: '25%', color: 'text.secondary' }}>地址:</Typography><Typography variant="body2" sx={{ width: '80%', fontWeight: 500 }}>{selectedCustomer.address ?? '無'}</Typography></ListItem>
          <ListItem sx={{ py: 0.5, display: 'flex', justifyContent: 'space-between' }}><Typography variant="body2" sx={{ width: '25%', color: 'text.secondary' }}>line:</Typography><Typography variant="body2" sx={{ width: '80%', fontWeight: 500 }}>{selectedCustomer.line ?? '無'}</Typography></ListItem>
...
const CustomersPage: FC = () => {
  ...
  const columns = [
    { field: 'code', headerName: '會員編號', flex: 1.3 },
    { field: 'name', headerName: '會員姓名', flex: 1.3 },
    { field: 'idCardNumber', headerName: '身分證', flex: 1.6 },
    { field: 'birthdate', headerName: '出生年月日', flex: 1.5, valueGetter: (params: any) => formatDateToYYYYMMDD(params?.value) },
    { field: 'phone', headerName: '電話', flex: 1.3 },
    { field: 'level', headerName: '會員等級', flex: 1 },
    {
      field: 'actions',
      headerName: '操作',
      flex: 2,
