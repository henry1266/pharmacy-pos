...
const mapAxiosError = (error: AxiosError): ApiError => {
  ...
  if (error.response) {
    ...
    switch (status) {
      case 400:
        return createApiError(ApiErrorType.VALIDATION_ERROR, message ?? '請求參數有誤', status, error);
      case 401:
        return createApiError(ApiErrorType.UNAUTHORIZED, message ?? '未授權，請重新登入', status, error);
      case 403:
        return createApiError(ApiErrorType.FORBIDDEN, message ?? '禁止存取資源', status, error);
      case 404:
