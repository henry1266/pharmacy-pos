...
interface DataVisualizationProps {
  ...
  monthlyData: MonthlyData;
  chartType: number;
  onChartTypeChange: (event: React.SyntheticEvent, newValue: number) => void;
}
...
export const DataVisualization: FC<DataVisualizationProps> = ({
  ...
  const data = prepareChartData(monthlyData);
  const pieData = data.filter(item => item.金額 > 0);
  
  // 渲染圖表 - 根據類型選擇適當的圖表
  const renderChart = (type: number): React.ReactNode => {
    switch (type) {
      case 0: // 長條圖
      ...
      case 2: // 圓餅圖
        return <PieChartComponent data={pieData} colors={CHART_COLORS} />;
      default:
        return null;
    }
  };
  ...
  return (
    <Box sx={{ mt: 3 }}>
      ...
      <Box sx={{ mb: 2 }}>
        <Tabs value={chartType} onChange={onChartTypeChange} centered>
          <Tab label="長條圖" />
          <Tab label="折線圖" />
          <Tab label="圓餅圖" />
        </Tabs>
